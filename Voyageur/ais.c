...
void recupParams(int argc, char **argv,
                 ...
                 int *nbClones, int *nbNouveaux, int *type_meilleur_indv, int *type_muteac)
{
 int printUsage=0;
 ...
 if (argc==8) {
   ...
   if (sscanf(argv[6],"%d",type_meilleur_indv)!=1)
   {
    fprintf(stderr,"\n%s n'est pas un entier\n",argv[5]);
    printUsage=1;
   }
   else
   if (sscanf(argv[7],"%d",type_muteac)!=1)
   {
    fprintf(stderr,"\n%s n'est pas un entier\n",argv[5]);
    printUsage=1;
   }
...
int main(int argc, char** argv)
{
 ...
 int type_meilleur_indv = TYPEMEILLEURINVD;
 int type_muteac = TYPEMUTEAC;
 ...
 if (argc!=1) /* Si il y a des arguments */
 {
  recupParams(argc,argv,&nbIndividus,&nN,&dD,
              &nbGenerations,&nbGenerationsInjection,
              &nbClones,&nbNouveaux, &type_meilleur_indv, &type_muteac);
 }
 ...
 fprintf(stderr,"type_muteac=%d\n",type_muteac);
 fprintf(stderr,"type_meilleur_indv=%d\n",type_meilleur_indv);
 fprintf(stderr,"*********\n\n");
 ...
 while (tour!=nbGenerations)
 {
  ...
  clonageMeilleurs(&p);
  mutationClones(&p, type_muteac);
  selectionMeilleursEtClonesMutes(&p, type_meilleur_indv);
  mutationMoinsBons(&p, type_muteac);
